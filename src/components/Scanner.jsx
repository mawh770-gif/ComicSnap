    ...
    <div style={{ padding: '20px' }}>
      ...
      {scannedComicData && (
        <div style={{ border: scannedComicData.metadata?.details.imageSource === 'AI_RECOGNITION' ? '2px solid orange' : '1px solid #ccc', padding: '15px', borderRadius: '8px' }}>
          ...
          {/* The unique_inventory_id (SKU) is generated in the backend now, so we only display the base code */}
          <p>
            <strong>Base Code:</strong> {scannedComicData.barcodeData.publisherCode} {scannedComicData.barcodeData.titleCode}
            {scannedComicData.barcodeData.priceCode} {scannedComicData.barcodeData.checkDigit}
          </p>
          ...
          <div style={{ marginTop: '15px' }}>
            <button onClick={handleConfirmSave} disabled={status.includes('Finalizing')}>
              Confirm Save to {isAIRecognition ? 'STAGING' : 'INVENTORY'}
            </button>
